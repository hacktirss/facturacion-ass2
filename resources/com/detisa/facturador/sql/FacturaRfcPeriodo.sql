...
SELECT 
    ...
    FROM (
        ...
        SELECT 
            ...
            inv.inv_cproducto cproducto,
            cantidad,
            ROUND( unitario/( 1+iva ), 4 ) preciou,
            CAST( iva AS DECIMAL( 11, 6 ) ) factoriva,
            CAST( 0.00 AS DECIMAL( 11, 6 ) ) factorieps,
            ROUND( cantidad * ROUND( unitario/( 1+iva ), 4 ), 2 ) subtotal,
            ROUND( cantidad * ROUND( unitario/( 1+iva ), 4 ), 2 ) baseiva,
            ROUND( cantidad * ROUND( unitario/( 1+iva ), 4 )  * iva, 2 ) iva,
            0.00 importeieps,
            total
        FROM vtaditivos
        JOIN inv ON inv.id = vtaditivos.clave
        JOIN cli ON cli.id = vtaditivos.cliente
        WHERE true
                AND uuid = '-----'
                AND total > 0
                AND cantidad > 0
